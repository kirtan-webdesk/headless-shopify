// Sets a Metaobject definition's Storefront access to PUBLIC_READ, by type.
// Generalized from update-homepage-metaobject-access.mjs to work for any
// type (e.g. site_page). Reads credentials from .env at runtime only.
// Run with: node scripts/update-metaobject-access.mjs <type>
...
if (!type) {
  console.error('Usage: node scripts/update-metaobject-access.mjs <metaobject_type>');
  process.exit(1);
}
...
if (!defId) {
  console.error(`No metaobject definition found for type "${type}"`);
  process.exit(1);
}
